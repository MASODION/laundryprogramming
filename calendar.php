...
class CalendarObj {
  ...
  function modify($string) {
    ...
    return new CalendarDay(date('Y', $ts), date('m', $ts), date('d', $ts), date('H', $ts), date('i', $ts), date('s', $ts));
  }
...
class CalendarDay extends CalendarObj {
  ...
  function hours() {
    ...
    while($obj->int() == $this->int()) {
      if($obj->hour()->int() >= 8 && $obj->hour()->int() <= 28) $array[] = $obj->hour();
      $obj = $obj->plus('2hour');    
    }
      foreach($array as $a) {
          $a->washmachines = 5;
